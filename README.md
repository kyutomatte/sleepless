...
1. [Download Sleepless_0.1.2_aarch64.dmg](https://github.com/kyutomatte/sleepless/releases/download/v0.1.2/Sleepless_0.1.2_aarch64.dmg)를 다운로드합니다.
2. DMG를 열고 안내 화면처럼 `Sleepless.app`을 `Applications` 폴더로 드래그합니다.
3. `Applications` 폴더에서 Sleepless를 실행합니다.
...
처음 실행이 막히면 `완료`를 누른 뒤 시스템 설정의 `개인정보 보호 및 보안` 화면 아래쪽에서 `그래도 열기`를 선택합니다. 이 안내는 DMG 설치 화면에도 표시됩니다.

Sleepless installs a small privileged helper the first time awake mode is enabled. This helper is used only to run the AC-power `pmset disablesleep` change that macOS requires for closed-lid wake behavior.
...
Privileged helper를 완전히 삭제하려면 awake mode를 먼저 끈 뒤 아래 명령을 실행합니다.

```bash
sudo rm -f /Library/PrivilegedHelperTools/app.mac.sleepless.pmset-helper
```

v0.1.1 이전 테스트 빌드를 설치한 적이 있다면 이전 helper가 남아 있을 수 있습니다. 아래 명령으로 함께 정리할 수 있습니다.
...
- Confirm the privileged helper path is `/Library/PrivilegedHelperTools/app.mac.sleepless.pmset-helper`.
- Note whether the build is signed/notarized or unsigned.

## Security

Please see [SECURITY.md](SECURITY.md) for supported versions, reporting guidance, and the privileged-helper security model.
